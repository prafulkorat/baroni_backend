...
export const listMyAvailabilities = async (req, res) => {
    try {
        ...
        // Don't merge - return daily and weekly availabilities separately
        // Each availability will have its own isDaily and isWeekly flags
        const formattedItems = items.map(item => {
            const itemObj = item.toObject ? item.toObject() : item;
            return {
                ...itemObj,
                // Ensure flags are properly set based on what's in database
                isDaily: Boolean(itemObj.isDaily),
                isWeekly: Boolean(itemObj.isWeekly)
            };
        });
        ...
        return res.status(200).json({
            ...
            data: formattedItems.map(item => sanitize(item))
        });
    } catch (error) {
